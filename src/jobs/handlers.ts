...
import { refundPaymentForOrder } from "../services/paymentsService";
import { allowedTransitions, logReturnStatusChange, type ReturnStatus } from "../services/returnsService";
import { logAudit } from "../utils/audit";
import { logger } from "../utils/logger";
import {
  cancelAutoCancelOrder,
  enqueueAutoCancelOrder,
  enqueueStartReturnWindow,
  enqueueEmail
} from "./enqueue";
import { logOrderStatusChange, logOrderTimeline } from "../services/ordersService";
import { emitDomainEvent } from "../services/eventsService";
import { fetchRazorpayOrder } from "../services/paymentsService";
import { markOrderPaid } from "../services/ordersService";
import { applyRefundAdjustment } from "../services/settlementService";
import { createFinanceAlert } from "../services/financeAlertsService";
import { markProductsOutOfStockIfNeeded } from "../services/productLifecycleService";
import { invalidatePattern } from "../utils/cache";
import { incrementMetric } from "../utils/metrics";
import { processEligibleSettlements } from "../services/settlement.service";
import {
  assignCourier,
  createShipment,
  schedulePickup,
...
export const autoCancelUnpaidOrders = async () => {
  ...
  try {
    ...
    for (const row of rows) {
      ...
      await logOrderTimeline(client, row.id, "CANCELLED", "system", {
      ...
      await emitDomainEvent(
        "order.cancelled",
        { orderId: row.id, actorType: "system", actorId: null },
        client
      );
    }
...
export const autoCancelOrderIfUnpaid = async (orderId: string) => {
  const client = await db.connect();
  try {
    await client.query("BEGIN");
    const { rows } = await client.query(
    ...
    if (rows[0]) {
      ...
      await logOrderTimeline(client, rows[0].id, "CANCELLED", "system", {
        reason: "auto_cancel_unpaid"
      });
      await logAudit({
        entityType: "order",
      ...
      await emitDomainEvent(
        "order.cancelled",
        { orderId: rows[0].id, actorType: "system", actorId: null },
        client
      );
    }
...
export const autoExpireReturnWindow = async () => {
...
export const autoCloseDisputedReturns = async () => {
  const { rows } = await db.query(
    `SELECT id, status
     FROM return_requests
     WHERE status = 'DISPUTED'
       AND updated_at < NOW() - ($1 || ' days')::interval`,
    [env.DISPUTE_AUTO_CLOSE_DAYS]
  );

  let closed = 0;
  for (const row of rows) {
    if (!allowedTransitions.DISPUTED.includes("ADMIN_REJECTED")) {
      continue;
    }
...
export const reservationCleanup = async () => {
  const { rowCount } = await db.query(
...
export const deliverNotificationJob = async (deliveryId: string) => {
  const { deliverNotification, markDeliveryAttempt } = await import(
...
export const createShipmentJob = async (orderId: string) => {
  const client = await db.connect();
  try {
    ...
    const weightKg = Number(
      (shippingAddress.weightKg as number | undefined) ??
        (shippingAddress.weight as number | undefined) ??
        0.5
    );
    const dimensions = {
      length: Number((shippingAddress.length as number | undefined) ?? 10),
      breadth: Number((shippingAddress.breadth as number | undefined) ?? 10),
      height: Number((shippingAddress.height as number | undefined) ?? 5)
    };
    ...
    for (const item of itemsResult.rows) {
      ...
      const existing = groups.get(key) ?? {
        ...
        pickupAddress: (item.shiprocket_pickup_address as Record<string, unknown> | null) ?? null,
        subtotal: 0,
        items: []
      };
    ...
    for (const sellerGroup of groups.values()) {
      if (!sellerGroup.pickupName?.trim() || !sellerGroup.pickupAddress) {
        throw new Error(
          `Seller ${sellerGroup.sellerId} has no Shiprocket pickup address configured`
        );
      }

      const existingShipment = await client.query(
        `SELECT id, shiprocket_order_id, shiprocket_shipment_id, awb_code, courier_name, pickup_scheduled_at, shipment_status
         FROM order_shipments
         WHERE order_id = $1 AND seller_id = $2
         ORDER BY shipped_at DESC NULLS LAST
         LIMIT 1`,
        [orderId, sellerGroup.sellerId]
      );
      const shipmentRow = existingShipment.rows[0] as
        | {
            id: string;
      ...
      if (!awbCode) {
        const assigned = await assignCourier(shiprocketOrderId);
      ...
      if (!pickupScheduledAt) {
        const pickup = await schedulePickup(shiprocketOrderId);
      ...
      await logOrderTimeline(client, orderId, "SHIPMENT_CREATED", "shiprocket", {
    ...
    await invalidatePattern(`cache:orders:tracking:${orderId}`);
    return { status: "created", shipments };
  } catch (error) {
    logger.error({ err: error, orderId }, "Shiprocket shipment automation failed");
    throw error;
  } finally {
    client.release();
  }
...
export const syncTrackingForActiveShipments = async () => {
  const { rows } = await db.query(
    `SELECT os.id,
            os.order_id,
            os.awb_code,
  ...
  for (const shipment of rows) {
    try {
      const tracking = await fetchTracking(String(shipment.awb_code));
      ...
      const latest = latestResult.rows[0] as { status?: string; event_time?: string } | undefined;
      ...
      logger.error(
        { err: error, shipmentId: shipment.id, awbCode: shipment.awb_code },
...
export const sendEmailJob = async (input: {
  to: string;
  template:
  ...
  const { getEmailProvider, RetryableEmailError } = await import(
  ...
  try {
    ...
    if (env.EMAIL_PROVIDER === "sendgrid" && !result.skipped) {
      logger.info({ template: input.template, to: input.to }, "[EMAIL_SENT]");
      incrementMetric("email_sent_total");
    }
    if (env.EMAIL_PROVIDER === "sendgrid" && result.skipped && result.reason !== "provider_disabled") {
      logger.error({ template: input.template, to: input.to }, "[EMAIL_FAILED]");
      incrementMetric("email_failed_total");
    }
    return result;
  } catch (error) {
    if (error instanceof RetryableEmailError) {
      logger.warn({ template: input.template, to: input.to, statusCode: error.statusCode }, "[EMAIL_RETRY]");
      incrementMetric("email_retry_total");
      throw error;
    }
    logger.error({ template: input.template, to: input.to, err: error }, "[EMAIL_FAILED]");
    incrementMetric("email_failed_total");
...
export const batchNotifications = async () => {
  ...
  try {
    ...
    const { rows: groups } = await client.query(
      `SELECT user_id, batch_key, COUNT(*)::int AS count
       FROM notifications
    ...
    for (const group of groups) {
      ...
      const summaryId = summaryRows[0]?.id as string | undefined;
      if (!summaryId) {
        continue;
      }
      ...
      await client.query(
        `UPDATE notifications
         ...
         WHERE id = ANY($1::uuid[])`,
        [originals.map((row: any) => row.id), summaryId]
      );
...
export const startReturnWindow = async (orderId: string) => {
  await db.query(
    `UPDATE orders
     SET delivered_at = COALESCE(delivered_at, NOW()),
         ...
         updated_at = NOW()
     WHERE id = $2`,
    [env.RETURN_WINDOW_DAYS, orderId]
  );
  return { orderId };
};

export const refundReturn = async (returnId: string) => {
  const client = await db.connect();
  try {
    await client.query("BEGIN");

    const { rows } = await client.query(
      `SELECT id, status, seller_decision, order_id
       FROM return_requests
       WHERE id = $1`,
      [returnId]
    );
    ...
    const allowed = allowedTransitions[current.status as keyof typeof allowedTransitions] ?? [];
    ...
    await refundPaymentForOrder(current.order_id);

    const financials = await client.query(
      `SELECT seller_payout_amount, refunded_at
    ...
    if (fin && !fin.refunded_at) {
      ...
      const sellerId = sellerResult.rows[0]?.seller_id as string | undefined;
      if (sellerId) {
        const adjustment = await applyRefundAdjustment(client, {
          sellerId,
        ...
        await logOrderTimeline(client, current.order_id, "ADJUSTMENT", "system", {
          amount: Number(fin.seller_payout_amount),
          type: "REFUND"
        });
      }
...
const reserveStockForOrder = async (orderId: string) => {
  const client = await db.connect();
  try {
    await client.query("BEGIN");
    const { rows: items } = await client.query(
      `SELECT oi.variant_color_id, oi.size, oi.quantity, vc.product_id
       FROM order_items oi
       INNER JOIN product_variant_colors vc ON vc.id = oi.variant_color_id
       WHERE order_id = $1
       ORDER BY oi.variant_color_id, oi.size`,
      [orderId]
    );
    ...
    for (const item of items) {
      ...
      const { rows } = await client.query(
        `SELECT stock
         ...
         FOR UPDATE`,
        [item.variant_color_id, item.size]
      );
      ...
      await client.query(
        `UPDATE product_variant_sizes
         SET stock = stock - $3
         WHERE variant_color_id = $1 AND size = $2`,
        [item.variant_color_id, item.size, item.quantity]
      );
    }

    await markProductsOutOfStockIfNeeded(client, Array.from(productIds));
    ...
    await logOrderTimeline(client, orderId, "CONFIRMED", "system");
...
export const processEvent = async (eventId: string) => {
  const client = await db.connect();
  try {
    await client.query("BEGIN");
    const { rows } = await client.query(
    ...
    switch (event.event_type) {
      case "order.created":
        if (payload.orderId) {
          await enqueueAutoCancelOrder(payload.orderId);
        }
        break;
      case "order.paid":
        if (payload.orderId) {
          await reserveStockForOrder(payload.orderId);
          await cancelAutoCancelOrder(payload.orderId);
          await emitDomainEvent("order.confirmed", {
          ...
          const emailResult = await db.query(
            `SELECT u.email AS customer_email, s.owner_user_id, so.email AS seller_email
             FROM orders o
             INNER JOIN users u ON u.id = o.user_id
             INNER JOIN shops s ON s.id = o.shop_id
      ...
      case "order.delivered":
        if (payload.orderId) {
          await enqueueStartReturnWindow(payload.orderId);
        }
        break;
...
export const reconcilePayments = async () => {
  const { rows } = await db.query(
    `SELECT pi.id, pi.order_id, pi.provider_order_id, o.total_amount
  ...
  let processed = 0;
  for (const row of rows) {
    const order = await fetchRazorpayOrder(row.provider_order_id);
    ...
    await markOrderPaid(row.order_id, "system", null, "reconciliation");
    processed += 1;
  }
...
export const runSellerPayouts = async () => {
  return {
...
export const processSettlementsJob = async () => {
  return processEligibleSettlements();
};

export const createPayoutBatchesJob = async () => {
  return {
    disabled: true as const,
    reason: "manual_payout_mode"
  };
...
export const reconcileFinance = async () => {
  const [capturedResult, refundsResult, payoutsResult, commissionResult] =
    await Promise.all([
      db.query(
        `SELECT COALESCE(SUM(amount), 0)::numeric AS captured
      ...
      db.query(
        `SELECT COALESCE(SUM(amount), 0)::numeric AS payouts
         FROM seller_payouts
         WHERE status = 'COMPLETED'`,
        []
      ),
      db.query(
        `SELECT COALESCE(SUM(platform_commission_amount), 0)::numeric AS commission
         FROM order_financials`,
        []
      )
    ]);
  ...
  await db.query(
    `UPDATE system_state
     SET last_reconciliation_at = NOW(),
         ...
         END
     WHERE id = 1`,
    [Math.abs(mismatch) > env.FINANCE_MISMATCH_THRESHOLD ? 1 : 0]
  );

  if (Math.abs(mismatch) > env.FINANCE_MISMATCH_THRESHOLD) {
    await logAudit({
      entityType: "finance",
...
export const runSellerRiskMonitor = async () => {
  ...
  const alerts: Array<{ sellerId: string; type: string }> = [];
  for (const seller of sellers.rows) {
    const [salesResult, refundResult, balanceResult] = await Promise.all([
      db.query(
        `SELECT COALESCE(SUM(of.order_total), 0)::numeric AS sales
    ...
    if (refundRate > env.FINANCE_REFUND_RATE_THRESHOLD) {
      await createFinanceAlert({
        type: "high_refunds",
    ...
    if (pending < -env.NEGATIVE_BALANCE_LIMIT) {
      await createFinanceAlert({
...
export const sendFinanceDigest = async () => {
  const [gmvResult, commissionResult, refundsResult, negativeSellersResult, mismatchResult] =
    await Promise.all([
      db.query(`SELECT COALESCE(SUM(order_total), 0)::numeric AS gmv FROM order_financials`, []),
      db.query(
        `SELECT COALESCE(SUM(platform_commission_amount), 0)::numeric AS commission
         FROM order_financials`,
        []
...
export const financeSafeRecoveryCheck = async () => {
  const [mismatchResult, negativeResult, refundsResult, capturedResult, criticalAlerts] =
    await Promise.all([
      db.query(
        `SELECT COALESCE(SUM(amount), 0)::numeric AS captured
         FROM payment_intents
  ...
  const valid =
    Math.abs(mismatch) <= env.FINANCE_MISMATCH_THRESHOLD &&
  ...
  const riskSellers = await db.query(
    `SELECT seller_id
     FROM seller_balance
...
export const revalidateSellerRisk = async (sellerId?: string) => {
  ...
  const sellers = sellerId
    ? await db.query(
        `SELECT seller_id, risk_reason
         FROM seller_balance
         WHERE seller_id = $1 AND risk_flag = TRUE`,
        [sellerId]
      )
    : await db.query(
        `SELECT seller_id, risk_reason
  ...
  let cleared = 0;
  for (const seller of sellers.rows) {
    const [ordersResult, financialsResult, alertsResult, balanceResult] =
      await Promise.all([
        db.query(
    ...
    if (ledgerOk && alertCount === 0 && !negative && reconcileOk) {
      ...
      await emitDomainEvent("seller_risk_resolved", {
...
export const scoreSellerRisk = async () => {
  const { scoreAllSellers } = await import("../services/riskEngineService");
  return scoreAllSellers();
};

export const scoreSellerRiskLegacy = async () => {
  const sellers = await db.query(
    `SELECT s.owner_user_id AS seller_id
     FROM shops s
     GROUP BY s.owner_user_id`,
  ...
  let scored = 0;
  for (const seller of sellers.rows) {
    const sellerId = seller.seller_id as string;
    const [refundsResult, ordersResult, cancelledResult, failedPaymentsResult, paymentsResult, payoutHoldResult] =
      await Promise.all([
        db.query(
          `SELECT COALESCE(SUM(ra.amount), 0)::numeric AS refunds
           FROM refund_adjustments ra
           WHERE ra.seller_id = $1
             AND ra.created_at >= NOW() - '7 days'::interval`,
          [sellerId]
        ),
        db.query(
          `SELECT COUNT(*)::int AS count, COALESCE(SUM(o.total_amount), 0)::numeric AS total
           ...
           INNER JOIN shops s ON s.id = o.shop_id
           WHERE s.owner_user_id = $1
             AND o.created_at >= NOW() - '7 days'::interval`,
          [sellerId]
        ),
        db.query(
          `SELECT COUNT(*)::int AS count
        ...
        db.query(
          `SELECT COUNT(*)::int AS count
           ...
           WHERE s.owner_user_id = $1
             ...
             AND pi.created_at >= NOW() - '7 days'::interval`,
          [sellerId]
        ),
        db.query(
          `SELECT COUNT(*)::int AS count
    ...
    const negativeBalance = await db.query(
      `SELECT pending_amount FROM seller_balance WHERE seller_id = $1`,
      [sellerId]
    );
    ...
    await db.query(
      `UPDATE seller_balance
       SET risk_watch = $2,
           ...
           END
       WHERE seller_id = $1`,
      [sellerId, riskLevel !== "normal", riskLevel === "critical", score, riskLevel === "normal"]
    );
...
export const monitorSellerIsolation = async () => {
  const sellers = await db.query(
    `SELECT sb.seller_id, sb.seller_financial_mode, sb.risk_score, sb.risk_below_threshold_since
  ...
  for (const seller of sellers.rows) {
    ...
    const currentMode = seller.seller_financial_mode as string;
    ...
    let nextMode = "NORMAL";
    if (riskScore >= env.RISK_SCORE_CRITICAL) {
      nextMode = "ISOLATED";
    } else if (riskScore >= env.RISK_SCORE_THRESHOLD) {
      nextMode = "MONITORED";
    }

...
import { emitDomainEvent } from "./eventsService";
import { applyRefundAdjustment } from "./settlementService";
import { convertReservationsForOrder } from "../modules/reservations/reservation.service";
import { refundPaymentForOrder } from "./paymentsService";
import { createFinanceAlert } from "./financeAlertsService";
...
export const logOrderStatusChange = async (
  ...
  await client.query(
    `INSERT INTO order_status_history
     ...
     VALUES ($1, $2, $3, $4, $5)`,
    [orderId, fromStatus, toStatus, changedBy ?? null, note ?? null]
  );
...
export const logOrderTimeline = async (
  client: QueryRunner,
  orderId: string,
  eventType:
    | "CREATED"
    ...
    | "SHIPMENT_STATUS",
  source: "webhook" | "manual" | "reconciliation" | "system" | "shiprocket",
  metadata?: Record<string, unknown>
) => {
  await client.query(
    `INSERT INTO order_timeline (order_id, event_type, source, metadata)
     VALUES ($1, $2, $3, $4)`,
    [orderId, eventType, source, JSON.stringify(metadata ?? {})]
  );
};
...
export const emitOrderEvent = async (
  ...
  event:
    ...
    | "order.cancelled"
    | "payment.captured",
  actorType: "customer" | "shop_owner" | "admin" | "system",
  actorId?: string | null
) => {
...
export const getOrderById = async (orderId: string) => {
  const { rows } = await db.query(
    `SELECT o.*, s.owner_user_id AS seller_id
     FROM orders o
...
export const markOrderPaid = async (
  ...
  actorType: "customer" | "shop_owner" | "admin" | "system",
  actorId?: string | null,
  source: "webhook" | "reconciliation" | "manual" = "webhook"
) => {
  ...
  try {
    ...
    try {
      await convertReservationsForOrder(client, orderId, current.user_id);
    } catch (error) {
      ...
      if (
        ...
        const refundMeta = await client.query(
          `SELECT pi.amount, pi.provider_payment_id, pi.provider_order_id
        ...
        const refundAmount = Number(
          refundMeta.rows[0]?.amount ?? orderTotalResult.rows[0]?.total_amount ?? 0
        );
        const gatewayRef =
          refundMeta.rows[0]?.provider_payment_id ??
        ...
        await logOrderStatusChange(
          ...
          "PAYMENT_STOCK_FAILED",
          actorId ?? null,
          "Payment captured but stock reservation expired",
          actorType
        );
        ...
        await client.query(
          `INSERT INTO order_refunds (order_id, amount, status, gateway_reference, reason)
           ...
           SET amount = EXCLUDED.amount,
               ...
               updated_at = NOW()`,
          [orderId, refundAmount, gatewayRef, "payment_stock_failed"]
        );

        const financials = await client.query(
          `SELECT of.order_id, of.seller_payout_amount, of.refunded_at, s.owner_user_id AS seller_id
        ...
        if (fin && !fin.refunded_at) {
          ...
          if (Number(fin.seller_payout_amount) > 0) {
            await applyRefundAdjustment(client, {
        ...
        await refundPaymentForOrder(orderId);
    ...
    await logOrderTimeline(client, orderId, "PAID", source);

    await emitOrderEvent(client, orderId, "payment.captured", actorType, actorId ?? null);
    await emitOrderEvent(client, orderId, "order.paid", actorType, actorId ?? null);
    await logAudit({
      entityType: "payment",
      entityId: orderId,
    ...
    try {
      ...
      const emailResult = await client.query(
        `SELECT o.id, o.total_amount, u.email AS customer_email, so.email AS seller_email
         FROM orders o
      ...
      const row: any = emailResult.rows[0];
      ...
      logger.warn({ err: e, orderId }, "Failed to enqueue order placed email");
    }

    // Shiprocket shipping automation (best-effort; never break order flow)
    try {
      const { enqueueShippingCreate } = await import("../jobs/enqueue");
      await enqueueShippingCreate(orderId);
    } catch (e: unknown) {
      logger.warn({ err: e, orderId }, "Failed to enqueue shipment automation");
    }
...
export const getOrdersForAdmin = async (
  ...
  const data: AdminOrderListItem[] = rows.map((row) => ({

...
exports.getRefundsSummary = exports.getLowStockProducts = exports.getTopProducts = exports.getSalesTrend = exports.getDashboardSummary = void 0;
const pool_1 = require("../../db/pool");
const getDashboardSummary = async (shopId, sellerId) => {
    const client = await pool_1.db.connect();
    try {
        await client.query("BEGIN");
        const [salesResult, ordersResult, refundsResult, pendingResult] = await Promise.all([
            client.query(`SELECT COALESCE(SUM(of.order_total), 0)::numeric AS total_sales
         FROM order_financials of
         INNER JOIN orders o ON o.id = of.order_id
         WHERE o.shop_id = $1
           AND o.payment_status = 'paid'`, [shopId]),
            client.query(`SELECT COUNT(*)::int AS total_orders
         FROM orders o
         WHERE o.shop_id = $1
           AND o.payment_status = 'paid'`, [shopId]),
            client.query(`SELECT COUNT(*)::int AS refund_count,
                COALESCE(SUM(r.amount), 0)::numeric AS refund_amount
         FROM order_refunds r
         INNER JOIN orders o ON o.id = r.order_id
         WHERE o.shop_id = $1
           AND r.status = 'COMPLETED'`, [shopId]),
            client.query(`SELECT pending_amount
         FROM seller_balance
...
const getSalesTrend = async (shopId, status) => {
    const statusClause = status === "ALL"
        ? "TRUE"
        : status === "DELIVERED"
            ? "o.status = 'DELIVERED'"
            : "o.payment_status = 'paid'";
    const { rows } = await pool_1.db.query(`WITH days AS (
...
const getTopProducts = async (shopId, status, limit, offset) => {
    const statusClause = status === "ALL"
        ? "TRUE"
        : status === "DELIVERED"
            ? "o.status = 'DELIVERED'"
            : "o.payment_status = 'paid'";
    const { rows } = await pool_1.db.query(`SELECT p.id AS product_id,
            p.name,
...
const getLowStockProducts = async (shopId) => {
    const { rows } = await pool_1.db.query(`SELECT id, name, quantity
     FROM products
     WHERE shop_id = $1
       AND quantity <= 3
     ORDER BY quantity ASC, updated_at DESC`, [shopId]);
    return rows.map((row) => ({
        productId: row.id,
        name: row.name,
...
const getRefundsSummary = async (shopId) => {
...
exports.getRefundsSummary = getRefundsSummary;
//# sourceMappingURL=sellerDashboard.service.js.map
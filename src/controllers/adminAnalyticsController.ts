...
import { getCache, getWithRefresh, setCache } from "../utils/cache";
...
const resolveRange = (query: z.infer<typeof rangeSchema>) => {
...
const applyOrderFilters = (
  input: { sellerId?: string; shopId?: string },
...
export const getAnalyticsOverview = async (req: Request, res: Response) => {
  ...
  console.log("HANDLER_FN_STRING:", getAnalyticsOverview.toString().slice(0, 120));
  ...
  const data = await getWithRefresh(cacheKey, 60, async () => {
    ...
    const refundParams: Array<string | number> = [from, to];
    const refundWhere = ["r.created_at >= $1", "r.created_at <= $2"];
    const refundJoins: string[] = [];
    ...
    const [ordersResult, refundsResult, payoutsResult, alertsResult, riskResult, stockResult, deliveryResult] =
      await Promise.all([
        db.query(
          `SELECT COUNT(*)::int AS order_count, COALESCE(SUM(o.total_amount), 0)::numeric AS gmv
           FROM orders o
           ${orderJoin}
           ${orderClause}`,
          params
        ...
        db.query(
          `SELECT COUNT(*)::int AS refund_count, COALESCE(SUM(r.amount), 0)::numeric AS refund_total
        ...
        db.query(
          `SELECT COALESCE(SUM(p.amount), 0)::numeric AS payout_total
        ...
        db.query(
          `SELECT COUNT(*)::int AS risky_sellers
           FROM seller_balance
           WHERE risk_flag = TRUE`,
          []
        ),
        db.query(
          `SELECT COUNT(*)::int AS stock_outs
           FROM products
...
export const getAnalyticsOrders = async (req: Request, res: Response) => {
  const query = rangeSchema.parse(req.query);
  const { from, to, range } = resolveRange(query);
  ...
  const response = {
    ...
    from,
    to,
    items: rows.map((row) => keysToCamel(row))
  };
  await setCache(cacheKey, response, 30);
  return res.json(response);
};

export const getAnalyticsRefunds = async (req: Request, res: Response) => {
  const query = rangeSchema.parse(req.query);
  const { from, to, range } = resolveRange(query);
  ...
  const response = {
    ...
    from,
    to,
    refunds: keysToCamel(rows[0] ?? {})
  };
  await setCache(cacheKey, response, 30);
  return res.json(response);
};

export const getAnalyticsQualityRisk = async (req: Request, res: Response) => {
  const query = rangeSchema.parse(req.query);
  const { from, to, range } = resolveRange(query);
  ...
  const [productResult, riskLevelResult, riskRatesResult] = await Promise.all([
    ...
    db.query(
      `SELECT AVG(chargeback_rate_7d)::numeric AS chargeback_rate_7d,
  ...
  const response = {
    ...
    riskRates: keysToCamel(riskRatesResult.rows[0] ?? {})
  };
  await setCache(cacheKey, response, 30);
  return res.json(response);
};

export const getAnalyticsNotifications = async (req: Request, res: Response) => {
  const query = rangeSchema.parse(req.query);
  const { from, to, range } = resolveRange(query);

...
exports.calculateSellerQuality = void 0;
const pool_1 = require("../db/pool");
const audit_1 = require("../utils/audit");
const clamp01 = (value) => Math.max(0, Math.min(1, value));
const clampScore = (value) => Math.max(0, Math.min(100, Math.round(value)));
const computeTier = (score) => {
    if (score >= 85)
        return "PLATINUM";
    if (score >= 70)
        return "GOLD";
...
const computeQualityScore = (inputs) => {
    const score = clamp01(inputs.deliverySuccessRate) * 0.25 +
...
const getQualityInputs = async (sellerId) => {
    const [ordersResult, deliveredResult, returnsResult, videoReturnsResult, repeatResult, ratingResult, volumeResult] = await Promise.all([
        pool_1.db.query(`SELECT COUNT(*)::int AS total
         FROM orders o
         INNER JOIN shops s ON s.id = o.shop_id
         WHERE s.owner_user_id = $1
           AND o.created_at >= NOW() - '90 days'::interval`, [sellerId]),
        pool_1.db.query(`SELECT COUNT(*)::int AS delivered
         FROM orders o
         INNER JOIN shops s ON s.id = o.shop_id
         WHERE s.owner_user_id = $1
           AND o.status IN ('DELIVERED', 'COMPLETED')
           AND o.created_at >= NOW() - '90 days'::interval`, [sellerId]),
        pool_1.db.query(`SELECT COUNT(*)::int AS returns
         FROM return_requests rr
         INNER JOIN orders o ON o.id = rr.order_id
         INNER JOIN shops s ON s.id = o.shop_id
         WHERE s.owner_user_id = $1
           AND rr.requested_at >= NOW() - '90 days'::interval`, [sellerId]),
        pool_1.db.query(`SELECT COUNT(*)::int AS video_returns
         FROM return_requests rr
         INNER JOIN orders o ON o.id = rr.order_id
         INNER JOIN shops s ON s.id = o.shop_id
         WHERE s.owner_user_id = $1
           ...
           AND rr.requested_at >= NOW() - '90 days'::interval`, [sellerId]),
        pool_1.db.query(`SELECT COUNT(*)::int AS repeat_customers
         FROM (
        ...
        pool_1.db.query(`SELECT COALESCE(STDDEV_POP(p.rating_avg), 0)::numeric AS rating_stddev
         FROM products p
         INNER JOIN shops s ON s.id = p.shop_id
         WHERE s.owner_user_id = $1
           AND p.rating_count > 0`, [sellerId]),
        pool_1.db.query(`WITH daily AS (
...
const calculateSellerQuality = async (sellerId) => {
    ...
    await pool_1.db.query(`INSERT INTO seller_quality_metrics
    ...
    await pool_1.db.query(`INSERT INTO seller_quality_history (seller_id, seller_quality_score, seller_tier, breakdown)
     VALUES ($1, $2, $3, $4)`, [sellerId, score, tier, JSON.stringify(breakdown)]);
    await pool_1.db.query(`UPDATE shops
     SET seller_quality_score = $2,
         seller_tier = $3,
         trust_badge = $4
     WHERE owner_user_id = $1`, [sellerId, score, tier, effects.trustBadge]);
...
exports.calculateSellerQuality = calculateSellerQuality;
//# sourceMappingURL=sellerQualityService.js.map
...
exports.getAdminPlatformRevenueGstReport = exports.exportAdminPlatformRevenueCsv = void 0;
const pool_1 = require("../db/pool");
const env_1 = require("../config/env");
const monthQuerySchema = /^\d{4}-(0[1-9]|1[0-2])$/;
const parseMonthStart = (month) => {
...
const exportAdminPlatformRevenueCsv = async (req, res) => {
    ...
    let offset = 0;
    while (true) {
        const result = await pool_1.db.query(`SELECT pl.created_at::date AS date,
              pl.seller_id,
       ...
       LIMIT $2 OFFSET $3`, [monthStart, pageSize, offset]);
...
exports.exportAdminPlatformRevenueCsv = exportAdminPlatformRevenueCsv;
const getAdminPlatformRevenueGstReport = async (req, res) => {
    const month = req.query?.month;
    const monthStart = parseMonthStart(month);
    if (!monthStart || typeof month !== "string") {
        return res.status(400).json({ error: "Invalid month format. Use YYYY-MM." });
    }
    const result = await pool_1.db.query(`SELECT COALESCE(SUM(amount), 0) AS taxable_value
     FROM platform_ledger
     WHERE type = 'REVENUE'
       AND date_trunc('month', created_at) = date_trunc('month', $1::date)`, [monthStart]);
    const taxableValue = Number(result.rows[0]?.taxable_value ?? 0);
    const gstRate = Number(env_1.env.PLATFORM_GST_RATE ?? 18);
...
exports.getAdminPlatformRevenueGstReport = getAdminPlatformRevenueGstReport;
//# sourceMappingURL=adminPlatformRevenueExport.controller.js.map